...
import org.ogcs.utilities.SmallFileReader;
...
public final class ChineseChessUtil {
    ...
    public static void verifyMap(Chessboard chessboard, String filePath) throws IOException {
        byte[] bytes = SmallFileReader.readFileBytes(filePath);
...
//                }
                ...
                try {
                    ...
                    switch (key) {
                        case '平': {
                            chessboard.move(from.getX(), from.getY(), toX, from.getY());
                            break;
                        }
                        case '进':
                        case '退':
                            switch (from.getType()) {
                                ...
                                case ChessConst.PIECE_PAO:
                                    ...
                                    chessboard.move(from.getX(), from.getY(), from.getX(), from.getY() + (key == '进' ? (side == 0 ? range : -range) : (side == 0 ? -range : range)));
                                    break;
                                case ChessConst.PIECE_MA: {
                                    int mX = toX - from.getX();
                                    int mY = Math.abs(mX) == 1 ? 2 : 1;
                                    chessboard.move(from.getX(), from.getY(), toX, from.getY() + (key == '进' ? (side == 0 ? mY : -mY) : (side == 0 ? -mY : mY)));
                                    break;
                                }
                                case ChessConst.PIECE_XIANG: {
                                    int mX = toX - from.getX();
                                    chessboard.move(from.getX(), from.getY(), toX, from.getY() + (key == '进' ? (side == 0 ? 2 : -2) : (side == 0 ? -2 : 2)));
                                    break;
                                }
                                case ChessConst.PIECE_SHI: {
                                    int mX = toX - from.getX();
                                    chessboard.move(from.getX(), from.getY(), toX, from.getY() + (key == '进' ? (side == 0 ? 1 : -1) : (side == 0 ? -1 : 1)));
                                    break;
                                }
                                case ChessConst.PIECE_BING:
                                case ChessConst.PIECE_JIANG:
                                    // 小兵 - 没有'退'
                                    chessboard.move(from.getX(), from.getY(), from.getX(), from.getY() + (key == '进' ? (side == 0 ? 1 : -1) : (side == 0 ? -1 : 1)));
                                    break;
                                default:
